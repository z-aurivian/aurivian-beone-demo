...
const MSL_OPTIONS = ['R. Kim — US Hematology East', 'J. Park — US Hematology West', 'S. Laurent — EU Hematology', 'M. Chen — US Oncology West', 'A. Okonkwo — US Hematology South', 'T. Weber — EU Oncology'];
...
const MSL_PERFORMANCE = [
  { msl: 'R. Kim',      territory: 'US Hematology East',  status: 'On track',     interactions: 22, target: 20, delta: '+2', qualityScore: 74, kitSignals: 3, kolsAtRisk: 2, note: 'Steady academic-centre engagement. Consistent KIT signal capture across 3 topics.' },
  { msl: 'J. Park',     territory: 'US Hematology West',  status: 'Volume gap',   interactions: 14, target: 20, delta: '−6', qualityScore: 79, kitSignals: 1, kolsAtRisk: 1, note: 'Quality strong but interactions below target. Coverage review needed.' },
  { msl: 'S. Laurent',  territory: 'EU Hematology',       status: 'Excellent',    interactions: 24, target: 20, delta: '+4', qualityScore: 82, kitSignals: 3, kolsAtRisk: 1, note: 'High interaction quality. Strong pediatric evidence focus.' },
  { msl: 'M. Chen',     territory: 'US Oncology West',    status: 'On track',     interactions: 19, target: 20, delta: '−1', qualityScore: 76, kitSignals: 5, kolsAtRisk: 2, note: 'Switch messaging delivery improving month-on-month.' },
  { msl: 'A. Okonkwo',  territory: 'US Hematology South', status: 'Quality gap',  interactions: 21, target: 20, delta: '+1', qualityScore: 61, kitSignals: 2, kolsAtRisk: 3, note: 'Volume adequate, quality below target. KIT alignment coaching needed.' },
  { msl: 'T. Weber',    territory: 'EU Oncology',         status: 'Needs support',interactions: 12, target: 20, delta: '−8', qualityScore: 58, kitSignals: 2, kolsAtRisk: 4, note: 'Both volume and quality below target. Escalated to director.' },
];
...
const TERRITORY_LIST = ['US Hematology East', 'US Hematology West', 'EU Hematology', 'US Oncology West', 'US Hematology South', 'EU Oncology'];
...
const TERRITORY_KIT_COVERAGE = {
  'US Hematology East':  [true,  true,  false, false, false],
  'US Hematology West':  [true,  false, false, false, false],
  'EU Hematology':       [true,  true,  true,  false, false],
  'US Oncology West':    [true,  true,  false, false, false],
  'US Hematology South': [true,  true,  true,  false, false],
  'EU Oncology':         [false, false, false, false, false],
};

const TERRITORY_BRIEFS = {
  'US Hematology East': 'US Hematology East is performing steadily — 22 interactions against a target of 20, quality score 74. R. Kim has strong academic-centre engagement and consistent KIT signal capture across 3 topics. Two tier-1/2 KOLs are at moderate engagement risk. Pediatric evidence remains an active listening theme this cycle.',
  'US Hematology West': 'US Hematology West is showing a volume gap — only 14 interactions against a target of 20. Quality score is strong at 79. One tier-1 KOL at risk. J. Park has captured the highest per-interaction quality in the team but needs territory coverage expansion. Patient advocacy signals are emerging.',
  'EU Hematology': 'EU Hematology is the strongest-performing territory this cycle — 24 interactions, quality score 82. S. Laurent is driving KIT signal coverage across 3 topics. Pediatric evidence (LP5) is the standout theme: 3 MSL interactions captured this cycle alone. One tier-1 KOL has shown a strong positive alignment shift this quarter — see Scientific Alignment Shift below. No KOL engagement gaps.',
  'US Oncology West': 'US Oncology West is on track with 19 interactions and quality score 76. Switch messaging delivery (MO2) is improving. 5 KIT signals captured — highest in any territory this cycle. Two tier-1 KOLs are at engagement risk (>45 days since last contact). Switch inertia remains the dominant theme: "if it\'s not broken" appeared in 3 separate field reports.',
  'US Hematology South': 'US Hematology South shows a quality gap — 21 interactions (above target) but quality score of 61, below the 75 threshold. A. Okonkwo has strong congress-sourced signal capture but KIT alignment in MSL interactions is inconsistent. Three KOLs at engagement risk. C5 in gMG is the strongest awareness topic with the highest avg stage nationally (3.2).',
  'EU Oncology': 'EU Oncology requires immediate support — 12 interactions (60% of target) and quality score 58. T. Weber has 4 tier-1/2 KOLs with no substantive contact in the past 60 days. KIT coverage dots are below all other territories. Escalated to field director. Director brief and coaching plan recommended before next cycle.',
};
...
function TerritoryView() {
  const [territory, setTerritory] = useState('US Hematology East');
  const [openTopic, setOpenTopic] = useState(null);
